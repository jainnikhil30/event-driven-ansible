...
class EDAControllerModule(AnsibleModule):
    url = None
    AUTH_ARGSPEC = dict(
        eda_controller_host=dict(required=False, aliases=['eda_host'], fallback=(env_fallback, ['EDA_CONTROLLER_HOST'])),
        eda_controller_username=dict(required=False, aliases=['eda_username'], fallback=(env_fallback, ['EDA_CONTROLLER_USERNAME'])),
        eda_controller_password=dict(no_log=True, aliases=['eda_password'], required=False, fallback=(env_fallback, ['EDA_CONTROLLER_PASSWORD'])),
        validate_certs=dict(type='bool', aliases=['eda_verify_ssl'], required=False, fallback=(env_fallback, ['EDA_CONTROLLER_VERIFY_SSL'])),
        request_timeout=dict(type='float', required=False, fallback=(env_fallback, ['EDA_CONTROLLER_REQUEST_TIMEOUT'])),
    )
    short_params = {
    ...
    def __init__(self, argument_spec=None, direct_params=None, error_callback=None, warn_callback=None, **kwargs):
        ...
        try:
            ...
            self.fail_json(msg="Unable to parse eda_controller_host as a URL ({1}): {0}".format(self.host, e))
        ...
        try:
            ...
            self.fail_json(msg="Unable to resolve eda_controller_host ({1}): {0}".format(self.url.hostname, e))
...
class EDAControllerAPIModule(EDAControllerModule):
    ...
    def get_one(self, endpoint, name=None, allow_none=True, check_exists=False, **kwargs):
    ...
    def make_request(self, method, endpoint, *args, **kwargs):
        ...
        try:
            ...
            # A 405 means we used a method that isn't allowed. Usually this is a bad request.
            elif he.code == 405:
    ...
    def delete_if_needed(self, existing_item, endpoint, on_delete=None, auto_exit=True):
        ...
        #   2. The response from EDA from calling the delete on the endpont. It's up to you to process the response and exit from the module
    ...
    def create_if_needed(self, existing_item, new_item, endpoint, on_create=None, auto_exit=True, item_type='unknown'):
        ...
        #    2. The response from EDA from calling the patch on the endpont. It's up to you to process the response and exit from the module
        # Note: common error codes from the EDA API can cause the module to fail
        response = None
        if not endpoint:
    ...
    def fields_could_be_same(old_field, new_field):
        ...
        if isinstance(old_field, dict) and isinstance(new_field, dict):
            ...
            for key in new_field.keys():
                if not EDAControllerAPIModule.fields_could_be_same(old_field[key], new_field[key]):
                    return False
    ...
    def update_if_needed(self, existing_item, new_item, endpoint, item_type, on_update=None, auto_exit=True):
        ...
        #    2. The response from EDA from patching to the endpoint. It's up to you to process the response and exit from the module.
        # Note: common error codes from the EDA API can cause the module to fail
        response = None
        if existing_item:
            ...
            self.json_output['id'] = item_id
            if needs_patch:
                response = self.patch_endpoint(endpoint, **{'data': new_item, 'id': item_id})
                if response['status_code'] == 200:
                    # compare apples-to-apples, old API data to new API data
                    # but do so considering the fields given in parameters
                    self.json_output['changed'] |= self.objects_could_be_different(existing_item, response['json'],
