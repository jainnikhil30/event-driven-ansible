...
options:
  ...
  validate_certs:
    ...
    - If value not set, will try environment variable C(EDA_CONTROLLER_VERIFY_SSL) and then config files
    type: bool
  request_timeout:
    description:
    - Specify the timeout Ansible should use in requests to the eda controller host.
    - Defaults to 10s, but this is handled by the shared module_utils code
    - If value not set, will try environment variable C(EDA_CONTROLLER_REQUEST_TIMEOUT) and then config files
    type: float
'''
